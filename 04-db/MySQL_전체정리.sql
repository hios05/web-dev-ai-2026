...
--    영화들의 모든 컬럼 조회
-- 직접 작성해보기
SELECT * FROM film WHERE rating = 'PG' or 'R'
AND rental_rate >= 3;
...
-- 직접 작성해보기
SELECT film_id, title, rental_rate, original_language_id FROM film 
WHERE rental_rate * rental_duration >= 15
...
WHERE payment_id >= '2005-07-01' AND amount > 0
...
--    (10글자보다 길면 뒤에 '...' 붙이기)
-- 직접 작성해보기
SELECT concat(substr(title, 1, 10), '...') title_preview
FROM film;
